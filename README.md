...
- 특징
    ...
    - POJO(Plain Old Java Object) 방식의 프레임 워크
        - 일반적인 J2EE 프레임워크에 비해 특정 라이브러리를 사용할 필요가 없어서 개발이 쉬우며, 기존 라이브러리의 지원이 용이
    - IOC(Inversion of Control) 지원
        - 컨트롤의 제어권이 개발자가 아니라 프레임워크에 있음을 말한다. 즉, 객체의 생성부터 모든 생명주기의 관리까지 객체의 제어권이 바뀐 것이다.
    - DI(Dependency Injection) 지원
        - 어떤 객체가 필요로 하는 객체를 자기 자신이 직접 생성하는것이 아니라, 외부에 있는 다른곳에서 자신이 필요로 하는 객체를 주입받는 것이다.
